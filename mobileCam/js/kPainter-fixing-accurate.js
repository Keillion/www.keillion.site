...
var KPainter = function(initSetting){
    ...
    var cropGesturer = new function(){
        ...
        var x0, y0, moveTouchId, orientX, orientY, bcbr, 
            cvsCx, cvsCy, cvsW, cvsH,
            left, top, width, height,
            rectCx, rectCy, rectW, rectH, rectMinL, rectMinT, rectMinR, rectMinB;
        ...
        var getInfo = function(){
            ...
            bcbr = box.contentBoxRect();
            getCvsInfo();
            rectW = parseFloat(kPainterCroper[0].style.width);
            rectH = parseFloat(kPainterCroper[0].style.height);
            rectCx = (parseFloat(kPainterCroper[0].style.left) + parseFloat(kPainterCroper[0].style.right)) / 2;
            rectCy = (parseFloat(kPainterCroper[0].style.top) + parseFloat(kPainterCroper[0].style.bottom)) / 2;
            rectMinL = Math.max(-bcbr.width/2, cvsCx - cvsW / 2);
            rectMinT = Math.max(-bcbr.height/2, cvsCy - cvsH / 2);
            rectMinR = Math.min(bcbr.width/2, cvsCx + cvsW / 2);
            rectMinB = Math.min(bcbr.height/2, cvsCy + cvsH / 2);
        };
        var getCvsInfo = function(){
            var tsf = $(mainCvs).getTransform();
            //var zoom = mainCvs.kPainterZoom;
            cvsCx = (parseFloat(mainCvs.style.left)+parseInt(mainCvs.style.right)) / 2;
            cvsCy = (parseFloat(mainCvs.style.top)+parseInt(mainCvs.style.bottom)) / 2;
            if(0 != tsf.a*tsf.d && 0 == tsf.b*tsf.c){
        ...
        var setCropBox = function(){
            var horOff = ((leftRela + rightRela) / 2 + cvsCenterX) * cvsW;
            var verOff = ((topRela + bottomRela) / 2 + cvsCenterY) * cvsH;
            kPainterCroper[0].style.left = (horOff - absoluteCenterDistance)+'px';
            kPainterCroper[0].style.right = (-horOff - absoluteCenterDistance)+'px';
            kPainterCroper[0].style.top = (verOff - absoluteCenterDistance)+'px';
            kPainterCroper[0].style.bottom = (-verOff - absoluteCenterDistance)+'px';
            kPainterCroper[0].style.width = Math.round((rightRela - leftRela) * cvsW)+'px';
            kPainterCroper[0].style.height = Math.round((bottomRela - topRela) * cvsH)+'px';
        ...
        var setCropRectArea = kPainter.setCropRectArea = function(l,t,r,b){
            ...
            leftRela = l, topRela = t, rightRela = r, bottomRela = b;
            getInfo();
